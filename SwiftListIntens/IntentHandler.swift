...
extension IntentHandler: SelectCharacterIntentHandling {
    ...
    func provideCharacterOptionsCollection(for intent: SelectCharacterIntent, with completion: @escaping (INObjectCollection<GameCharacter>?, Error?) -> Void) {
        let menus = ["扫一扫", "收付款", "出行", "健康码", "卡包", "会员积分", "余额宝", "账单查询"]
        let characters: [GameCharacter] = menus.map {
            let character = GameCharacter(identifier: $0, display: $0)
            character.name = $0
            return character
        }
